...
class LicenseValidationService
{
    ...
    {
        try {
            // Dung lượng S3 (bytes) được sudo.vn đồng bộ và gửi kèm trong payload license
            $size = $data['size'] ?? null;
            unset($data['size']);
            ...
            $this->clearCacheAndRefresh($size);
     ...
     *
     * @param int|string|null $size Dung lượng S3 (bytes) lấy từ payload license, nếu có
     */
    protected function clearCacheAndRefresh($size = null)
    {
        try {
            ...
            if (class_exists('\Sudo\Base\Supports\SettingStore')) {
                ...
                $settingStore->set('media_size_calculator', is_numeric($size) ? (int) $size : null);
                $settingStore->save();
            }
