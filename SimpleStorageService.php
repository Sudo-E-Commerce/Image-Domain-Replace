...
class SimpleStorageService
{
    ...
    public function getCurrentStorageSize()
    {
        try {
            ...
            // Với driver S3, dung lượng đã được đồng bộ từ sudo.vn và lưu sẵn vào
            // setting media_size_calculator, nên đọc trực tiếp từ DB cho nhanh
            // thay vì tự list toàn bộ bucket.
            if ($driver === 's3') {
                $cachedSize = $this->getMediaSizeCalculatorFromDB();
                if ($cachedSize !== null) {
                    return $cachedSize;
                }
            }
    ...
    /**
     * Lấy dung lượng từ setting media_size_calculator (đồng bộ sẵn từ sudo.vn)
     *
     * @return int|null Size in bytes, hoặc null nếu chưa có/không hợp lệ
     */
    private function getMediaSizeCalculatorFromDB()
    {
        try {
            $setting = null;
            ...
            if (Schema::hasTable('settings')) {
                $setting = DB::table('settings')
                    ->where('key', 'media_size_calculator')
                    ->first();
            }
            ...
            if (!$setting && Schema::hasTable('options')) {
                $setting = DB::table('options')
                    ->where('name', 'media_size_calculator')
                    ->first();
            }
            ...
            Log::error('Failed to get media_size_calculator from DB: ' . $e->getMessage());
            return null;
        }
    }
